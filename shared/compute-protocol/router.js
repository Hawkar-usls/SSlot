...
const SECRETISH_KEYS = new Set([
  'secret', 'password', 'private_key', 'privateKey', 'wallet_seed', 'seed_phrase', 'mnemonic',
  'api_key', 'apiKey', 'app_key', 'appKey', 'access_token', 'refresh_token', 'authorization'
]);
...
export function createRoutingPlan({ plan_id = 'default', allocations, policy = {} }) {
  ...
  assertNoSecrets(policy, 'routing_plan.policy');
  ...
  const callerPolicy = clone(policy);
  delete callerPolicy.scheduling_basis;
  delete callerPolicy.game_event_weighting;
...
export function validatePlanAgainstRegistry(plan, registry, { taskType } = {}) {
  ...
  if (plan.policy?.fallback_provider_id) {
    const fallback = registry.get(plan.policy.fallback_provider_id);
    if (!fallback || !fallback.enabled) throw new Error(`FALLBACK_PROVIDER_UNAVAILABLE:${plan.policy.fallback_provider_id}`);
    if (taskType && !fallback.task_types.includes(taskType)) throw new Error(`FALLBACK_PROVIDER_TASK_MISMATCH:${plan.policy.fallback_provider_id}`);
  }
  if (plan.policy?.game_event_weighting !== 'FORBIDDEN') throw new Error('GAME_EVENT_WEIGHTING_MUST_BE_FORBIDDEN');
  if (plan.policy?.scheduling_basis !== 'CONSENT_DEVICE_POLICY_AND_PROVIDER_CAPACITY') throw new Error('INVALID_SCHEDULING_BASIS');
  return true;
}
...
export function selectProvider(plan, registry, { taskType, cursor = 0 } = {}) {
  ...
  // Cursor MUST be generated by the compute scheduler from neutral scheduling state/capacity,
  // never from wager, spin, RNG result, loss, win, bet size, bonus or player-risk state.
